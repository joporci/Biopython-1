...
# The first print command in the loop, displays an output of the main menu
while option != 'Q':
...
# This part of the program deals with displaying the information of the protein from the PDB file		
	...
	if option == 'H':
		if file_loaded ==True:
			...
			order = raw_input('order by: ').lower()
...
#								temp_file.write(line)
			...
			if manipulation == 'N':
				...
				if add == 'S':
					...
					if chain == 'A':
						print 'This position corresponds to the amino acid ' + aaA[int(endSeqNum)]
						sheet = sheet + [(chain,sheet_id,sub_structure, int(initSeqNum), int(endSeqNum))]
					else:
